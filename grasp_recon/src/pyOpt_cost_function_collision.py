...
class CostFns:
    def __init__(self,immediate_cost,final_cost,grad,grad_final,robot_model,x_d,u0):
        ...
        #self.gradient_x_final=grad_x_final
    ...
    def objectiveFunction(self,q):
        ...
        if(self.robot.T>1):
            ...
            cost_arr_p = cost_arr[:,0]
            cost_arr_c = cost_arr[:,1]
            # print(cost_arr[:,1])
            # cost for shortest joint trajectory:
            cost=sum(cost_arr_p)
            #print cost
            cost_final = self.final_cost(x_d[-1], u[-1], self.robot)
            cost+=cost_final[0]
            cost_arr_c = np.append(cost_arr_c,cost_final[1])
    ...
    def objectiveGradient(self,q):
        ...
        if(self.robot.T>1):
            ...
            gradient_arr = np.array(gradient_arr)
            gradient_arr_p = gradient_arr[:,0]

            gradient=np.ravel(gradient_arr_p)
            gradient_final = self.gradient_u_final(x_d[-1],u[-1],self.robot)
            gradient = np.append(gradient, gradient_final[0])
        ...
        '''
        gradient for contraints
        '''
        gradient_arr_collision = gradient_arr[:, 1]
        gradient_arr_collision = list(gradient_arr_collision)
        gradient_collision = np.ravel(gradient_arr_collision)
        gradient_collision = np.append(gradient_collision, gradient_final[1])
        gradient_collision = list(gradient_collision)

        return [gradient,gradient_collision]
