...
#from handArmModel_baxter import handArmModel_baxter
from handArmModel_baxter_left import handArmModel_baxter
import PyKDL
...
class baxter_hand_model:
    ...
    def collision_link_poses(self,u):
        ...
        r_link_poses=[]
        # Add lbr4 link poses
        #for full arm collision checking
        for i in range(8):
    ...
    def link_poses_matrix(self, u, i):
        if i<8:
            return self.arm_link_pose_matrix(u,i)
        elif i == 8:
            return self.hand_link_pose_matrix(u, 0, 0)
        elif i == 9:
            return self.hand_link_pose_matrix(u, 1, 0)
        elif i == 10:
            return self.hand_link_pose_matrix(u, 0, 1)
        elif i == 11:
            return self.hand_link_pose_matrix(u, 1, 1)


    def arm_link_pose_matrix(self, u, link_num):
        if link_num <= 6:
            T = self.kdlModel.FK_link(u[0:link_num + 1], link_num)
        else:
            T = self.kdlModel.FK_link(u[0:7], link_num)
        return T

    def hand_link_pose_matrix(self,u,link_num,f_index):
        #u_=list(np.ravel(u[0:7]))+list(np.ravel(u[7+f_index*4:7+f_index*4+link_num+1]))
        if f_index == 0:
            u_ = list(np.ravel(u[0:8]))
        elif f_index == 1:
            joints_0_right = np.append(u[0:7], u[8])
            u_ = list(np.ravel(joints_0_right[0:8]))
        #u_ = list(np.ravel(u[0:8]))
    ...
    def link_jacobian(self,u,link_num,f_index):
        ...
        if(f_index==-1):
            if link_num<=6:
                ...
                J = self.kdlModel.Jacobian_link(u[0:7], link_num)
                return J
        else:
            # link_num=0-3,
            #u_=list(np.ravel(u[0:7]))+list(np.ravel(u[7+f_index*4:7+f_index*4+link_num+1]))
